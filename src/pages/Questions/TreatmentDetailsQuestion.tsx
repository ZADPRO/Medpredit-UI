...
import { IonToast, useIonAlert } from "@ionic/react";
...
const TreatmentDetailsQuestion: React.FC<TreatmentDetailsQuestionProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      axios
        .post(
          `${import.meta.env.VITE_API_URL}/getTreatmentDetails`,
          {
        ...
        .then((response: any) => {
          ...
          if (data.status) {
            ...
            // Map the treatment details to the QuestionSet structure
            const fetchedQuestionSets: QuestionSet[] = data.treatementDetails.map((item: any) => ({
              id: item.refTDId,
              nameOfMedicine: item.refTDMedName || "",
              category: item.refTDCat || null,
              strength: item.refTDStrength || null,
              roa: item.refTDROA || null,
              relationToFood: item.refTDRTF || null,
              morningdosage: item.refTDMorningDosage || null,
              morningtime: item.refTDMorningDosageTime ? new Date(item.refTDMorningDosageTime) : null,
              afternoondosage: item.refTDAfternoonDosage || null,
              afternoontime: item.refTDAfternoonDosageTime ? new Date(item.refTDAfternoonDosageTime) : null,
              eveningdosage: item.refTDEveningDosage || null,
              eveningtime: item.refTDEveningDosageTime ? new Date(item.refTDEveningDosageTime) : null,
              nightdosage: item.refTDNightDosage || null,
              nighttime: item.refTDNightDosageTime ? new Date(item.refTDNightDosageTime) : null,
              monthsduration: item.refTDDurationMonth || null,
              yearsduration: item.refTDDurationYear || null,
            }));

            // Update the questionSets state with the fetched data
            setQuestionSets(fetchedQuestionSets);
            setQuestionIndex(fetchedQuestionSets.length > 0 ? fetchedQuestionSets.length - 1 : 0);
  ...
  const handleRemoveSet = (index: number) => {
    ...
    try {
      axios
        .post(
          `${import.meta.env.VITE_API_URL}/deleteTreatmentDetail`,
          {
                ...
                <div className="questions">
                  ...
                  >
                    <InputNumber
                      id="hrsInputLeft"
                      style={{ width: "40%" }}
                      value={set.monthsduration || undefined}
                      onChange={(e) =>
