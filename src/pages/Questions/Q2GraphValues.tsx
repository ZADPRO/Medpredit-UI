...
const Q2Graphvalues: React.FC<GraphValuesProps> = ({
    label,
    onEdit,
}) => {
    ...
    const addItem = () => {
        const allFieldsFilled = data.every(
            (item) => item.date !== null && item.number !== null && item.twoHours !== null
        );

        if (allFieldsFilled) {
            setData([...data, { id: null, date: null, number: null, twoHours: null, flag: "ui" }]);
        }
    };
